...
await page.evaluate(() => {
  ...
  window.__sim.replies["region def us west california centralcoast slo slonorth"] = "Err - unknown jump: nope";
});
page.once("dialog", (d) => d.accept());
await page.click("#btn-push");
await page.waitForFunction(
...
await page.evaluate(() => {
  window.__sim.log.length = 0;
  delete window.__sim.replies["region def us west california centralcoast slo slonorth"];
});
