...
check("nearest area is Paso Robles North County",
...
check("sent exactly the generated commands + verification",
  JSON.stringify(sentCmds) === JSON.stringify([
    ...
    "region def west ca cc slo prb", "region save", "region get prb"]),
...
await page.evaluate(() => {
  ...
  window.__sim.replies["region def west ca cc slo prb"] = "Err - unknown jump: nope";
});
page.once("dialog", (d) => d.accept());
await page.click("#btn-push");
await page.waitForFunction(
...
await page.evaluate(() => {
  window.__sim.log.length = 0;
  delete window.__sim.replies["region def west ca cc slo prb"];
});
