#!/usr/bin/env node
/*
 * Regenerates data/outlines.json — the boundary shapes the map draws.
 *
 *   node scripts/build-outlines.mjs path/to/california-counties.geojson
 *
 * The output is committed, so this only needs running if the boundaries are
 * ever replaced. Contributors adding a place never touch it.
 *
 * Shapes are a library keyed by name, not a level of the region tree: a place
 * in data/regions.yaml claims one with `outline: Del Norte`. Today they are
 * California's counties, which is why the input is a counties GeoJSON, but
 * nothing here assumes that — drop in city boundaries and they work the same.
 *
 ...
 * looking like California, and emits one SVG path per shape.
 */
...
if (!SRC) {
  console.error("usage: build-outlines.mjs <boundaries.geojson>");
  process.exit(1);
}
...
for (const feature of geo.features) {
  ...
  // "biggest part" logic downstream gets the part people think of as the shape.
  ...
  shapes.push({ name, d, area: Math.round(parts.reduce((s, p) => s + ringArea(p), 0)) });
}
...
const all = shapes.flatMap((c) => extent(c.d));
const PAD = 4;
...
/* ---------- emit ----------
 * The notice travels inside the file rather than in a comment above it: JSON
 * has no comments, and anyone fetching this from outside the repo should get
 * the attribution along with the shapes.
 */

const out = printJson({
  notice: "Generated by scripts/build-outlines.mjs — do not edit by hand. " +
    "Boundaries: US Census TIGER/Line, via the click_that_hood project " +
    "(https://github.com/codeforgermany/click_that_hood, MIT). Census boundary data is a " +
    "work of the US government and in the public domain.",

  // Feed straight into the SVG viewBox attribute, in this order.
  viewBox: { x: minX, y: minY, width, height },

  // Coordinates below are viewBox units, not degrees. Anything plotting a
  // lat/lon onto this map has to use the same numbers.
  projection: {
    formula: "x = (lon - lon0) * cos0 * k ; y = (lat1 - lat) * k",
    inverse: "lon = x / (cos0 * k) + lon0 ; lat = lat1 - y / k",
    lon0: LON0,
    lat1: LAT1,
    k: K,
    cos0: Number(COS0.toFixed(9))
  },

  // Claimed by name from data/regions.yaml's `outline` field.
  shapes: shapes.map((c) => ({ name: c.name, d: c.d }))
}) + "\n";

writeFileSync(new URL("../data/outlines.json", import.meta.url), out);
